...
export async function POST(request: Request) {
  ...
  try {
    ...
    const expectedHeaderColumnCount = headerRow.length; // <--- 列数を更新 (4列)
    ...
    try {
        ...
        if (getHeaderRes.data.values && getHeaderRes.data.values.length > 0) {
            ...
            if (existingHeader.length === expectedHeaderColumnCount && 
                ...
                existingHeader[3] === headerRow[3]) { // <--- ファイル名のヘッダー確認を追加
    ...
    for (const file of files) {
      ...
      if (file.name.toLowerCase().endsWith('.heic') || (file.mimeType && file.mimeType.toLowerCase().includes('heic'))) {
          ...
          parsedDataResult = { 
              ...
              userNotes: '',
              sourceFileName: file.name, // ファイル名を設定
      ...
      try {
        ...
        parsedDataResult = { 
            textInfo: `ファイル処理エラー (${file.name}): ${fileProcessingError.message}`, 
            userNotes: '',
            sourceFileName: file.name,
            driveFileLink: file.webViewLink || ''
        };
